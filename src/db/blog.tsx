...
export async function fetchBlogPost(route: string) {
    const query = await db.query(
        'SELECT name, content, poster.displayname AS username, poster.image AS image FROM posts WHERE route = $route',
        { route },
    );
...
export async function fetchBlogPosts(page: string) {
    ...
    if (pg > 0) {
        const posts = await db.query(
            'SELECT name, route FROM posts ORDER BY time DESC LIMIT 10 START $pg;',
            { pg },
        );
        return posts[0].result;
    }
...
export async function createBlogPost({ name, content, poster }: NewPost) {
    ...
    const duplicatePostName = await db.query('SELECT * FROM posts WHERE route = $route;', {
        route,
    });
    ...
    const post = await db.query(
        `INSERT INTO posts (route, name, poster, time, content) VALUES ($route, $name, $user, time::now(), $content)`,
        {
            route,
            name,
